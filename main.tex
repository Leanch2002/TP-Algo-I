...
\Tipo{EdR}
    ...
    \requiereLargo{f = c \yLuego \\ examenValido(solucion) \yLuego \\ cantValidaEstudiantes(a, cantEstudiantes) }
    \aseguraLargo{
        (\norm{res.aula} = f) \yLuego
...
% Si el area del aula es un numero par -> el maximo valor que puede tomar cantEstudiantes es |a|/2
% Si el area es impar -> el maximo es (|a|+1)/2
...
% Son aux pq devuelven algo, lo separe en 2 aux pq era mas facil
\auxLargo{cuantosEstudiantes}{a: Aula}{\Z}{
    \sum_{n = 0}^{\norm{a}-1}{cuantosEstudiantesFila(a[i])} 
}

% Si hay alumno en fila[i] suma uno sino nada.
\auxLargo{cuantosEstudiantesFila}{fila: \seq{Examen}}{\Z}{
    \sum_{n = 0}^{\norm{a}-1}{ifthenelse(hayAlumno(fila[i]), 1, 0)} 
}
